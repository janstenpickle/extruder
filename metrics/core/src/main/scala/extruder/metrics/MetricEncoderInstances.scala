...
trait MetricEncoderInstances extends MapEncoderInstances {
  ...
  implicit def traversableThrowableEncoder[F[_]: Applicative: ExtruderErrors, FF[A] <: TraversableOnce[A], S <: Settings]: Encoder[
    F,
    S,
    FF[Throwable],
    Metrics
  ] = Encoder.make[F, S, FF[Throwable], Metrics] { (path, settings, values) =>
  ...
  implicit def traversableEncoder[F[_]: Applicative, T, FF[A] <: TraversableOnce[A], S](
    implicit encoder: Encoder[F, S, T, Metrics],
    monoid: Monoid[Metrics]
  ): Encoder[F, S, FF[T], Metrics] =
    Encoder.make[F, S, FF[T], data.Metrics] { (path, settings, values) =>
  ...
  ): Encoder[F, S, Map[K, V], Metrics] =
    Encoder.make[F, S, Map[K, V], Metrics] { (path, settings, values) =>
  ...
  implicit def numericMapEncoderObjectKey[F[_]: Monad: ExtruderErrors, K <: Product with Serializable, T, S <: MetricSettings](
    ...
    enc: Encoder[F, Settings, K, Map[String, String]],
    lp: LowPriority
  ): Encoder[F, S, Map[K, T], Metrics] =
    Encoder.make[F, S, Map[K, T], Metrics] { (path, settings, values) =>
      val newPath = settings.pathElementsAsStrings(path)
  ...
  implicit def numericMapEncoderMapKey[F[_]: Applicative: ExtruderErrors, KK: Show, KV: Show, T, S <: Settings](
    implicit inj: Inject[Numbers, T]
  ): Encoder[F, S, Map[Map[KK, KV], T], Metrics] =
    Encoder.make[F, S, Map[Map[KK, KV], T], Metrics] { (path, settings, values) =>
      val newPath = settings.pathElementsAsStrings(path)
  ...
  implicit def numericMapEncoderTupleKey[F[_]: Applicative: ExtruderErrors, KK: Show, KV: Show, T, S <: Settings](
    implicit inj: Inject[Numbers, T]
  ): Encoder[F, S, Map[(KK, KV), T], Metrics] =
    Encoder.make[F, S, Map[(KK, KV), T], Metrics] { (path, settings, values) =>
      val newPath = settings.pathElementsAsStrings(path)
  ...
  implicit def metricValuesEncoder[F[_]: Applicative: ExtruderErrors, K: Show, T, V[A] <: MetricValue[A], S <: Settings](
    implicit inj: Inject[Numbers, T]
  ): Encoder[F, S, MetricValues[V, K, T], Metrics] =
    Encoder.make[F, S, MetricValues[V, K, T], Metrics] { (path, settings, mv) =>
      val newPath = settings.pathElementsAsStrings(path)
  ...
  implicit def metricValuesMapKey[F[_]: Applicative: ExtruderErrors, KK, KV, T, V[A] <: MetricValue[A], S <: Settings](
    ...
    showK: Show[KK],
    showKK: Show[KV]
  ): Encoder[F, S, MetricValues[V, Map[KK, KV], T], Metrics] =
    Encoder.make[F, S, MetricValues[V, Map[KK, KV], T], Metrics] { (path, settings, mv) =>
      val newPath = settings.pathElementsAsStrings(path)
  ...
  implicit def metricValuesTupleKey[F[_]: Applicative: ExtruderErrors, KK, KV, T, V[A] <: MetricValue[A], S <: Settings](
    ...
    showKK: Show[KK],
    showKV: Show[KV]
  ): Encoder[F, S, MetricValues[V, (KK, KV), T], Metrics] =
    Encoder.make[F, S, MetricValues[V, (KK, KV), T], Metrics] { (path, settings, mv) =>
      val newPath = settings.pathElementsAsStrings(path)
  ...
  implicit def numericEncoder[F[_]: Applicative: ExtruderErrors, T, S <: Settings](
    implicit inj: Inject[Numbers, T]
  ): Encoder[F, S, T, Metrics] =
    Encoder.make[F, S, T, Metrics] { (path, settings, value) =>
  ...
  implicit def metricValueEncoder[F[_]: Applicative: ExtruderErrors, T, V[A] <: MetricValue[A], S <: Settings](
    implicit inj: Inject[Numbers, T],
    refute: Refute[Encoder[F, S, V[T], Metrics]]
  ): Encoder[F, S, V[T], Metrics] = Encoder.make[F, S, V[T], Metrics] { (path, settings, mv) =>
    MetricKey[F](settings.pathElementsAsStrings(path), Some(mv.metricType))
      .map(key => Metrics.single(key, Coproduct[Numbers](mv.value)))
  }

  implicit def timerEncoder[F[_]: Applicative: ExtruderErrors, S <: Settings]: Encoder[F, S, TimerValue[Long], Metrics] =
    Encoder.make[F, S, TimerValue[Long], Metrics] { (path, settings, tv) =>
  ...
  implicit def durationEncoder[F[_]: Applicative: ExtruderErrors, S <: Settings]: Encoder[
    F,
    S,
    FiniteDuration,
    Metrics
  ] =
    Encoder.make[F, S, FiniteDuration, Metrics] { (path, settings, dur) =>
      MetricKey[F](settings.pathElementsAsStrings(path), Some(MetricType.Timer))
