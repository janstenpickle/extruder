...
trait TypesafeConfigDecoderInstances extends Resolve { outer =>
  ...
  protected def resolveConfig[F[_], A](
    lookup: (List[String], Settings, Config) => F[Option[A]]
  )(implicit F: Monad[F], error: ExtruderErrors[F]): (List[String], Settings, Option[A], Config) => F[A] =
  ...
  implicit def dataObjectDecoder[F[_]](
    ...
    DecoderT.make[F, Settings, ConfigObject, Config](
      resolveConfig[F, ConfigObject]((path, settings, data) => lookup(_.getObject(settings.pathToString(path)), data))
    )
  ...
  implicit def dataListDecoder[F[_]](
    ...
    DecoderT.make[F, Settings, ConfigList, Config](
      resolveConfig[F, ConfigList]((path, settings, data) => lookup(_.getList(settings.pathToString(path)), data))
    )
  ...
  implicit def dataValueDecoder[F[_]](
    ...
    DecoderT.make[F, Settings, ConfigValue, Config](
      resolveConfig[F, ConfigValue]((path, settings, data) => lookup(_.getValue(settings.pathToString(path)), data))
    )
  ...
  implicit def traversableDecoder[F[_], A, FF[T] <: TraversableOnce[T], S <: Settings](
    ...
    DecoderT.make[F, S, FF[A], Config](
      (path, settings: Settings, default, data) =>
        lookup[F, List[String]](_.getStringList(settings.pathToString(path)).asScala.toList, data).flatMap(
          listOpt =>
  ...
  implicit def traversableObjectDecoder[F[_], A, FF[T] <: TraversableOnce[T], S <: Settings](
    ...
    DecoderT.make[F, S, FF[A], Config] { (path, settings, default, data) =>
      lookup[F, List[Config]](_.getConfigList(settings.pathToString(path)).asScala.toList, data)
  ...
  implicit def typesafeConfigPrune[F[_]](implicit F: MonadError[F, Throwable]): Prune[F, Settings, Config] =
    new Prune[F, Settings, Config] {
      override def prune(path: List[String], settings: Settings, data: Config): F[Option[(List[String], Config)]] =
        lookup[F, ConfigObject](_.getObject(settings.pathToString(path)), data).map(_.map { co =>
          val pruned =
  ...
  implicit def typesafeConfigStringReader[F[_]](
    ...
    new StringReader[F, Settings, Config] {
      override def lookup(path: List[String], settings: Settings, data: Config): F[Option[String]] =
        outer.lookup[F, String](_.getString(settings.pathToString(path)), data)
    }
  ...
  implicit def typesafeConfigHasValue[F[_]](implicit F: ApplicativeError[F, Throwable]): HasValue[F, Settings, Config] =
    new HasValue[F, Settings, Config] {
      override def hasValue(path: List[String], settings: Settings, data: Config): F[Boolean] =
