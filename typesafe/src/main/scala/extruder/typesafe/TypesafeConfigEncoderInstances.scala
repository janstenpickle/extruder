...
trait TypesafeConfigEncoderInstances {
  ...
  implicit def typesafeConfigTransform[F[_]](
  ...
  implicit def iterableObjectEncoder[F[_]: Monad, A, FF[T] <: IterableOnce[T]](
    ...
    Traverse[List]
      .sequence[F, Config](value.iterator.toList.map(encoder.write(List.empty, settings, _)))
      .map(c => List(ConfigTypes.nested(settings.pathElementListToString(path), c)))
  }

  implicit def iterableEncoder[F[_]: Applicative, A, FF[T] <: IterableOnce[T]](
    implicit shows: Show[A],
    neOpt: FF[A] <:!< Option[A]
  ): Encoder[F, Settings, FF[A], Config] =
    Encoder.make[F, Settings, FF[A], Config] { (path, settings, value) =>
      List(ConfigTypes(settings.pathElementListToString(path), value.iterator.map(shows.show).toList)).pure[F]
    }
}
