...
trait CombinedDecoderInstances {
  ...
  implicit def combinedDecoder[F[_], A, S0, S1, I0, I1](
    implicit ev0: Lazy[Decoder[F, S0, A, I0]],
  ...
  private def combineOrFallback[F[_]: FlatMap, A, S0, S1, I0, I1](combine: (A, A) => A)(
    ...
    default: Option[A],
    input: (I0, I1)
  )(implicit ev0: Lazy[Decoder[F, S0, A, I0]], ev1: Lazy[Decoder[F, S1, A, I1]], errors: ExtruderErrors[F]): F[A] = {
  ...
  implicit def combinedNonEmptyChainDecoder[F[_]: FlatMap, A, S0, S1, I0, I1](
    implicit ev0: Lazy[Decoder[F, S0, NonEmptyChain[A], I0]],
    ev1: Lazy[Decoder[F, S1, NonEmptyChain[A], I1]],
    errors: ExtruderErrors[F]
  ): Decoder[F, (S0, S1), NonEmptyChain[A], (I0, I1)] =
    Decoder.make(combineOrFallback[F, NonEmptyChain[A], S0, S1, I0, I1]((d0, d1) => d0 ++ d1))

  implicit def combinedOptionalDecoder[F[_]: FlatMap, A, S0, S1, I0, I1](
    implicit ev0: Lazy[Decoder[F, S0, Option[A], I0]],
    ev1: Lazy[Decoder[F, S1, Option[A], I1]],
    errors: ExtruderErrors[F]
  ): Decoder[F, (S0, S1), Option[A], (I0, I1)] =
    Decoder.make(combineOrFallback[F, Option[A], S0, S1, I0, I1]({
