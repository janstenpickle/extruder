...
trait ConfigSpec extends Specification with ScalaCheck with EitherMatchers with Discipline with ConfigValidationInstances {
  self: Encode with
        Encoders with
        PrimitiveEncoders with
        DerivedEncoders with
        EncodeTypes with
        Decode with
        Decoders with
        DecodeFromDefaultConfig with
        PrimitiveDecoders with
        DerivedDecoders with
        DecodeTypes =>
  ...
  val caseClassConfig: Map[Seq[String], String] = Map(
    Seq("CaseClass", "s") -> "string",
    Seq("CaseClass", "i") -> "1",
    Seq("CaseClass", "l") -> "1"
  )
  ...
  def testNumeric[T: Numeric](implicit encoder: Enc[ConfigValidation, T],
  ...
  def testType[T](gen: Gen[T])(implicit encoder: Enc[ConfigValidation, T],
                               decoder: Dec[ConfigValidation,T],
                               eitherEncoder: Enc[EitherErrors, T],
                               eitherDecoder: Dec[EitherErrors, T],
                               listEncoder: Enc[ConfigValidation, List[T]],
  ...
  def testList[T, F[T] <: TraversableOnce[T]](gen: Gen[F[T]])(implicit encoder: Enc[ConfigValidation, F[T]], decoder: Dec[ConfigValidation, F[T]]): Prop =
  ...
  def test[T](gen: Gen[T])(implicit encoder: Enc[ConfigValidation, T], decoder: Dec[ConfigValidation, T], teq: Eq[T], equals: Eq[ConfigValidation[T]], FM: FlatMap[ConfigValidation]): Prop =
    Prop.forAll(gen, namespaceGen) { (value, namespace) =>
  ...
  def testIO[T](gen: Gen[T])(implicit encoder: Enc[ConfigValidation, T],
                                      decoder: Dec[ConfigValidation, T],
                             teq: Eq[T],
                                      equals: Eq[IO[ConfigValidation[T]]]): Prop = Prop.forAll(gen, namespaceGen) { (value, namespace) =>
  ...
  def testAsync[T](gen: Gen[T])(implicit encoder: Enc[ConfigValidation, T],
                                decoder: Dec[ConfigValidation, T],
                                teq: Eq[T],
                                equals: Eq[IO[ConfigValidation[T]]],
                                ioc: IOConvert[Future]): Prop = Prop.forAll(gen, namespaceGen) { (value, namespace) =>
    def eqv(encoded: Future[ConfigValidation[OutputConfig]], decoded: InputConfig => Future[ConfigValidation[T]]): Boolean =
      equals.eqv(ioFlatMapForConfigValidation.flatMap(ioc.toIO(encoded))(e => ioc.toIO(decoded(e))), IO(value.validNel))
  ...
  def testUnsafe[T](gen: Gen[T])(implicit encoder: Enc[ConfigValidation, T], decoder: Dec[ConfigValidation, T], teq: Eq[T], equals: Eq[Try[T]], FM: FlatMap[Try]): Prop =
    Prop.forAll(gen, namespaceGen) { (value, namespace) =>
