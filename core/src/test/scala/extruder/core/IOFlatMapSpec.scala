...
abstract class IOFlatMapSpec[F[_], E](implicit IOF: IOFlatMap[F], A: ExtruderApplicativeError[F, E]) extends Specification with Discipline  {
  val tests: FlatMapTests[IOF[F, ?]] = FlatMapTests[IOF[F, ?]]

  implicit def IOFArb[A](implicit arb: Arbitrary[A]): Arbitrary[IO[F[A]]] = Arbitrary(Gen.oneOf(
