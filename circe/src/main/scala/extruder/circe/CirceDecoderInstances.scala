...
trait CirceDecoderInstances {
  implicit def circeHasValue[F[_]: Applicative, S <: CirceSettings]: HasValue[F, S, Json] = new HasValue[F, S, Json] {
    override def hasValue(path: List[String], settings: S, data: Json): F[Boolean] =
  ...
  implicit def fromCirceDecoder[F[_]: Applicative, S <: CirceSettings, A: Decoder](
    ...
    (focus(path, settings, json), default) match {
      ...
      case (None, None) =>
        error.missing(s"Could not find JSON value at '${settings.pathToString(path)}' and no default available")
    }
  }

  private def focus[S <: CirceSettings](path: List[String], settings: S, json: Json): Option[Json] =
    path match {
      case head :: tail =>
        tail
